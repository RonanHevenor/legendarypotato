#!/usr/bin/env python3
"""
Test script for generating a single animation frame
Much simpler and faster than the full animation
"""
...
async def test_single_frame(description: str, model: str = "qwen2.5:14b"):
    ...
    try:
        ...
        if len(frame_data["frame"]) != FRAME_SIZE:
            raise ValueError(f"Frame has {len(frame_data['frame'])} lines, expected {FRAME_SIZE}")

        for i, line in enumerate(frame_data["frame"]):
            if len(line) != FRAME_SIZE:
                raise ValueError(f"Line {i} has {len(line)} chars, expected {FRAME_SIZE}")
...
async def main():
    if len(sys.argv) < 2:
        print("Usage: test_single_frame.py <character_description> [model]")
        print("\nExamples:")
        print("  python test_single_frame.py 'red warrior'")
        print("  python test_single_frame.py 'blue wizard' qwen2.5:7b")
        sys.exit(1)
